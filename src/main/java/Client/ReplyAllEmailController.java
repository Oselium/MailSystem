...
public class ReplyAllEmailController {
    Errors errors;
    MailBox mailBox;
    @FXML
    ...
    public void initializeModel(MailBox mailBox) {
        ...
        String s = mailBox.getCurrentEmail().getSender();
        for (String to: mailBox.getCurrentEmail().getAddressee()) {
            if(!to.equals(mailBox.getUsername()))
              s = s + ", " + to;
        }

        to_textfield.setText(s);
